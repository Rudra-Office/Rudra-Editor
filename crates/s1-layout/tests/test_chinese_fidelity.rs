...
fn test_chinese_wrapping() {
    ...
    let body_id = model.next_id();
    model
        .insert_node(root, 0, Node::new(body_id, NodeType::Body))
        .unwrap();
    let p1_id = model.next_id();
    model
        .insert_node(body_id, 0, Node::new(p1_id, NodeType::Paragraph))
        .unwrap();
    let r1_id = model.next_id();
    model
        .insert_node(p1_id, 0, Node::new(r1_id, NodeType::Run))
        .unwrap();
    ...
    let t1_id = model.next_id();
    model
        .insert_node(r1_id, 0, Node::text(t1_id, &long_text))
        .unwrap();
    ...
    if let s1_layout::LayoutBlockKind::Paragraph { lines, .. } = &block.kind {
        ...
        assert!(
            lines.len() > 1,
            "Chinese text should have wrapped into multiple lines"
        );
