...
| WFC-07 | No builder depth/size limits in WASM — OOM via unlimited chaining | `ffi/wasm/src/lib.rs` | 4200-4250 | OPEN |
| WFC-08 | Facade error type loses original error context (converts to String) | `s1engine/src/error.rs` | — | OPEN |
...
| WFC-11 | No ABI stability guarantees for C FFI structs | `ffi/c/src/lib.rs` | 18-40 | OPEN |
| WFC-12 | CRDT state vector grows unbounded with cluster size | `s1-crdt/src/state_vector.rs` | — | OPEN |
| WFC-13 | No 3-way convergence tests for CRDT | `s1-crdt/src/collab.rs` | 664-900 | OPEN |
| WFC-14 | No error path tests for collaborative operations | `s1-crdt/src/collab.rs` | 609-1115 | OPEN |
